...
async function main() {
  ...
  for (const rt of roomTypeData) {
    const created = await prisma.roomType.upsert({
      where: { slug: rt.slug },
      // Re-seed should refresh prices + descriptive fields on existing types
      // so older seeded DBs can be brought up to date by re-running seed.
      update: {
        name: rt.name,
        tag: rt.tag,
        ...
        weekendMultiplier: rt.weekendMultiplier,
        image: rt.image,
      },
      create: { ...rt, images: [] },
    });
